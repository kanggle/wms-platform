...
- `TASK-BE-041-fix-TASK-BE-040.md` — Documentation-only follow-up: declare `Shipment.shipment_no` format `SHP-YYYYMMDD-NNNN` in `domain-model.md` §5 (mirrors how `Order.order_no` is documented in §1) so spec, contract example, and implementation all align (TASK-BE-040 AC-06 spec-side gap).
