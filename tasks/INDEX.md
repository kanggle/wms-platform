...
- `TASK-BE-048-admin-polish-bundle.md` — **TASK-BE-046 minor deviations 7건 묶음** (선행=BE-046 done, 동기=BE-047 ready). admin-service v1 follow-up 카탈로그 완전 정리. (#2) 5 dashboard controller 의 date-range 필터 (Order/Shipment/ASN/Adjustment/Alert). (#3) `X-Read-Model-Lag-Seconds` 응답 헤더 — `worstLagSeconds > 5` 시 emit, HandlerInterceptor + MeterRegistry. (#4) `OperationsController` per-topic Kafka lag — KafkaAdmin / AdminClient.listConsumerGroupOffsets 통합. (#5) Throughput counter raw SQL upsert (`INSERT ... ON CONFLICT DO UPDATE WHERE last_event_at < EXCLUDED`) 로 cross-pod race window 방지. (#6) `runbooks/db-role-grants.md` 신규 + column-level GRANT (alert_log.acknowledged_*) ops 절차. (#7) `inbound-events.md` ↔ `admin-events.md` split topic drift cross-reference (spec docs only, production 변경 0). (#8) `admin.query.cache.hit.rate` 메트릭 v1 always 0 javadoc 명시 (v2 Redis cache 활성). 단일 PR 권장 (≥ 30 file 시 2-3 sub-PR 분할). D4 churn freeze 면제 (`apps/admin-service/**` + project-internal docs). 분석=Opus 4.7 / 구현 권장=Sonnet 4.6 (7 minor wiring tweaks routine).
