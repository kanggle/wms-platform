...
> 본 문서는 `wms-platform` 의 모든 서비스가 GAP 를 표준 OIDC IdP 로 사용하는 방식을 1쪽으로 요약한다.
> ADR-001 (D1=A) 와 GAP 의 [consumer-integration-guide.md](../../../global-account-platform/specs/features/consumer-integration-guide.md) 의 wms 적용본이며,
...
- B2B 운영 사용자 (warehouse operator, master admin 등) 의 계정은 GAP 의 [account internal provisioning API](../../../global-account-platform/specs/contracts/http/internal/account-internal-provisioning.md) 를 통해 생성된다.
...
`wms.oauth2.allowed-issuers` 는 D2-b deprecation 윈도우 동안 SAS issuer 와 legacy `global-account-platform` issuer 양쪽을 허용한다 (TASK-BE-253 패턴).

---

## OAuth Clients (등록은 GAP V0010 마이그레이션에서 시드)
...
`<tenant>.<resource>.<action>` 명명 규칙. 모두 `tenant_id=wms` scope 으로 GAP 의 `oauth_scopes` 에 등록 (V0010).
...
OIDC 표준 scope (`openid`, `profile`, `email`, `offline_access`) 는 GAP 의 시스템 scope (V0008) 에서 자동 적용.
...
4. **Tenant 검증** — `TenantClaimValidator` 로 `tenant_id` claim 이 `wms` 인 경우만 통과. 그 외 (`fan-platform`, 향후 `erp`/`scm`/`mes`) → `tenant_mismatch` → 403 `TENANT_FORBIDDEN`.
...
`platform/error-handling.md` 의 envelope 형식 (`{ "error": { "code", "message", "timestamp", ... } }`) 을 따른다.
...
- [ ] D2-b deprecation 윈도우 종료 시 `wms.oauth2.allowed-issuers` 에서 `global-account-platform` 제거.
- [ ] GAP 의 `wms` 테넌트 등록 (admin-service [Tenant Lifecycle API](../../../global-account-platform/specs/contracts/http/admin-api.md#tenant-lifecycle-task-be-256)).
