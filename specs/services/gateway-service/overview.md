...
- **JWT validation** — OAuth2 Resource Server; validates `aud=wms-platform` + `tenant_id` claim. The tenant gate (`TenantClaimValidator`) is now **entitlement-trust dual-accept** (ADR-MONO-019 § D5): a token passes when **either** the legacy strict `tenant_id == wms` matches **or** the GAP-signed `entitled_domains` claim contains `wms`; rejection (`tenant_mismatch` → 403 `TENANT_FORBIDDEN`) requires **both** to fail. wms keeps strict legacy equality (no `*` wildcard); `entitled_domains` is read only from an RS256/JWKS-verified token so it is unforgeable. While GAP has not populated the claim it is absent → legacy-only (production net-zero); the legacy branch is removed in step 4 once GAP populates it (separate follow-up).
