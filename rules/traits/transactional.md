...
- 필수: `apps/auth-service/` (로그인·토큰 발급·refresh 회전), `apps/account-service/` (계정 상태 전이), `apps/admin-service/` (운영자 강제 작업)
- 조건부: `apps/security-service/` (자동 잠금 명령 발행 경로)
- 제외: 순수 읽기 경로(감사 조회, 로그인 이력 조회)
