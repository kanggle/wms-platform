package com.wms.notification.adapter.outbound.persistence.jpa;
...
import com.wms.notification.adapter.outbound.persistence.jpa.routing.RoutingRuleJpaEntity;
 ...
 * {@code @JdbcTypeCode(SqlTypes.JSON)}, otherwise Hibernate 6.x defaults
 * to OID parameter binding and writes garbage. This is the
 * TASK-SCM-INT-001b root cause #2 + TASK-SCM-BE-005 regression-guard
 * learning, ported to notification-service.
 *
 * <p>Removing the annotation on any covered field will fail this test —
 * exactly the contract the task spec calls for (Acceptance Criteria 6).
 */
class JsonbColumnRegressionGuardTest {

    static Stream<Class<?>> entitiesWithJsonb() {
        return Stream.of(
                NotificationDeliveryJpaEntity.class,
                NotificationOutboxJpaEntity.class,
                RoutingRuleJpaEntity.class);
    }
