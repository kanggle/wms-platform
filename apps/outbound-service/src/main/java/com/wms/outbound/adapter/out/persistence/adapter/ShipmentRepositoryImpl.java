 ...
 */
@Component("shipmentPersistenceImpl")
public class ShipmentRepositoryImpl implements ShipmentPersistencePort {

    private final ShipmentJpaRepository repo;

    public ShipmentRepositoryImpl(ShipmentJpaRepository repo) {
        this.repo = repo;
    }
    ...
    public Optional<Shipment> findById(UUID id) {
        return repo.findById(id).map(ShipmentRepositoryImpl::toDomain);
    }

    @Override
    @Transactional(readOnly = true)
    public Optional<Shipment> findByOrderId(UUID orderId) {
        return repo.findByOrderId(orderId).map(ShipmentRepositoryImpl::toDomain);
    }
