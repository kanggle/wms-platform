 ...
 */
@Component
public class SagaRepositoryImpl implements SagaPersistencePort {

    private final OutboundSagaRepository repo;
    private final Clock clock;

    public SagaRepositoryImpl(OutboundSagaRepository repo, Clock clock) {
        this.repo = repo;
        this.clock = clock;
    }
    ...
    public Optional<OutboundSaga> findById(UUID sagaId) {
        return repo.findById(sagaId).map(SagaRepositoryImpl::toDomain);
    }

    @Override
    @Transactional(readOnly = true)
    public Optional<OutboundSaga> findByOrderId(UUID orderId) {
        return repo.findByOrderId(orderId).map(SagaRepositoryImpl::toDomain);
    }
    ...
    public Optional<OutboundSaga> findByPickingRequestId(UUID pickingRequestId) {
        return repo.findByPickingRequestId(pickingRequestId)
                .map(SagaRepositoryImpl::toDomain);
    }
