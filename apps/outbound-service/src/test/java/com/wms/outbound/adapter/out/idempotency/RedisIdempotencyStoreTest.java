 ...
 * {@code outbound:idempotency:} prefix; callers pass the suffix
...
class RedisIdempotencyStoreTest {
    ...
    private static final String SUFFIX = "POST:/webhooks/erp/order:test-key";
    private static final String EXPECTED_ENTRY_KEY = "outbound:idempotency:" + SUFFIX;
    private static final String EXPECTED_LOCK_KEY = "outbound:idempotency:lock:" + SUFFIX;
    ...
    @Test
    @DisplayName("entry prefix matches idempotency.md §1.3 canonical shape (outbound:idempotency:)")
    void entryPrefixMatchesSpec() {
        assertThat(RedisIdempotencyStore.ENTRY_PREFIX).isEqualTo("outbound:idempotency:");
    }

    @Test
    @DisplayName("entry prefix is not the rejected short form outbound:idem:")
    void entryPrefixIsNotShortForm() {
        assertThat(RedisIdempotencyStore.ENTRY_PREFIX).isNotEqualTo("outbound:idem:");
    }
    ...
    @Test
    @DisplayName("tryAcquireLock writes lock key under outbound:idempotency:lock:")
    ...
    @Test
    @DisplayName("releaseLock deletes the canonical lock key")
