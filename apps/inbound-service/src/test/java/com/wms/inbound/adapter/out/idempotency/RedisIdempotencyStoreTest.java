 ...
 * {@code inbound:idempotency:} prefix; callers pass the suffix
 * {@code {method}:{path_hash}:{idempotency_key}}.
 *
 * <p>Mirrors the inventory-service {@code RedisIdempotencyStoreTest} pattern
 * (TASK-BE-025). The shorter {@code inbound:idem:} shape was rejected as
 * latent risk — this test is the gate that prevents regression.
 */
...
class RedisIdempotencyStoreTest {
    ...
    private static final String EXPECTED_ENTRY_KEY = "inbound:idempotency:" + SUFFIX;
    private static final String EXPECTED_LOCK_KEY = "inbound:idempotency:lock:" + SUFFIX;
    ...
    @Test
    @DisplayName("entry prefix matches idempotency.md §1.3 canonical shape (inbound:idempotency:)")
    void entryPrefixMatchesSpec() {
        assertThat(RedisIdempotencyStore.ENTRY_PREFIX).isEqualTo("inbound:idempotency:");
    }

    @Test
    @DisplayName("entry prefix is NOT the rejected short form inbound:idem:")
    void entryPrefixIsNotShortForm() {
        assertThat(RedisIdempotencyStore.ENTRY_PREFIX).isNotEqualTo("inbound:idem:");
    }
    ...
    @Test
    @DisplayName("tryAcquireLock writes lock key under inbound:idempotency:lock:")
