...
@Component
public class InspectionRepositoryImpl implements InspectionPersistencePort {

    private final InspectionJpaRepository inspectionRepo;
    private final InspectionDiscrepancyJpaRepository discrepancyRepo;

    public InspectionRepositoryImpl(InspectionJpaRepository inspectionRepo,
    ...
    public Optional<InspectionDiscrepancy> findDiscrepancyById(UUID discrepancyId) {
        return discrepancyRepo.findById(discrepancyId).map(InspectionRepositoryImpl::toDiscrepancyDomain);
    }
    ...
    private Inspection toDomain(InspectionJpaEntity e) {
        ...
        List<InspectionDiscrepancy> discs = e.getDiscrepancies().stream()
                .map(InspectionRepositoryImpl::toDiscrepancyDomain)
                .toList();
