...
@Component
public class PutawayRepositoryImpl implements PutawayPersistencePort {
    ...
    public PutawayRepositoryImpl(PutawayInstructionJpaRepository instructionRepo,
    ...
    public Optional<PutawayConfirmation> findConfirmationByLineId(UUID lineId) {
        return confirmationRepo.findByPutawayLineId(lineId).map(PutawayRepositoryImpl::toConfirmationDomain);
    }
    ...
    private PutawayInstruction toDomain(PutawayInstructionJpaEntity e) {
        ...
        List<PutawayLine> domainLines = lines.stream().map(PutawayRepositoryImpl::toLineDomain).toList();
