 ...
 *
 * <p>The dedupe insert runs inside the caller's outer transaction so the
 * dedupe row, the consumer's domain writes, and any outbox writes commit or
 * rollback together. If {@code work} throws, the transaction is rolled back
 * and the dedupe row is removed alongside, allowing a redelivery to retry.
 */
@Component
