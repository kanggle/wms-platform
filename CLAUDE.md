...
Stop immediately if any holds:

- No `PROJECT.md` is locatable for the current working context.
- `PROJECT.md` missing/unparseable, or declares `domain`/`trait` not in `rules/taxonomy.md`.
- A shared library file (under `platform/`, `rules/`, `.claude/`, `libs/`, `tasks/templates/`, `docs/guides/`) contains project-specific content (service names, API paths, domain entities) — Library vs Project boundary broken.
- A domain/trait rule file conflicts with a common rule without an explicit `## Overrides` block.
- The task is not in the appropriate `tasks/ready/`.
- Required specifications do not exist or conflict.
- Acceptance criteria are unclear.
- Required contracts are missing.
- The task requires architecture decisions not documented in specs.
- The target service's `Service Type` is undeclared or not in [`platform/service-types/INDEX.md`](platform/service-types/INDEX.md).
